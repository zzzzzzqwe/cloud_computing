...
# Задача
Создать VPC с публичными и приватными подсетями, развернуть RDS MySQL, настроить EC2, написать простое приложение, которое выполняет CRUD-операции.
# Цель
Научиться разворачивать инфраструктуру AWS (VPC, EC2, RDS), подключать приложение к базе данных и использовать мастер-реплику и реплику для чтения в разных сценариях.
...
# Вывод
В ходе работы была развернута инфраструктура в AWS: создана VPC, настроены публичные и приватные подсети, запущены EC2 и RDS, настроены security groups и подключение. Было разработано простое Java-приложение, выполняющее CRUD-операции с использованием master для записи и реплики для чтения.